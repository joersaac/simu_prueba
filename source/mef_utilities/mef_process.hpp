...
float calculate_local_jacobian(float x1, float y1, float z1, 
                        float x2, float y2, float z2, 
                        float x3, float y3, float z3,
                        float x4, float y4, float z4){
    return (x2 - x1)*(y3 - y1)*(z4 - z1) - (x2 - x1)*(y4 - y1)*(z3 - z1) - (x3 - x1)*(y2 - y1)*(z4 - z1) 
            + (x3 - x1)*(y4 - y1)*(z2 - z1) + (x4 - x1)*(y2 - y1)*(z3 - z1) - (x4 - x1)*(y3 - y1)*(z2 -z1);
}
...
void create_local_K(Matrix* K, int element_id, Mesh* M){
    ...
    //std::cout << "\t\tLocal matrix created for Element " << element_id+1 << ": "; K->show(); std::cout << "\n";
}
...
void create_local_b(Vector* b, int element_id, Mesh* M){
    ...
    //std::cout << "\t\tLocal vector created for Element " << element_id+1 << ": "; b->show(); std::cout << "\n";
}
...
void assembly(Matrix* K, Vector* b, Matrix* Ks, Vector* bs, int num_elements, Mesh* M){
    ...
    for(int e = 0; e < num_elements; e++){
        std::cout << "\tAssembling for Element " << e+1 << "...\n\n";
        ...
        //std::cout << "\t\t"; K->show(); std::cout << "\t\t"; b->show(); std::cout << "\n";
    }
}
