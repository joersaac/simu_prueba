...
void create_local_K(Matrix* K, int element_id, Mesh* M) {
    ...
    product_scalar_by_matrix(Volume/(J * J), &res3, 4, 4, K);
...
void create_local_b(Vector* b, int element_id, Mesh* M){
    ...
    b->set_value_on_pos(J/252, 0);
    b->set_value_on_pos(J/105, 1);
    b->set_value_on_pos((19*J)/2520, 2);
    b->set_value_on_pos(J/252, 3);
